...
task :import => [:environment] do
  ...
  CSV.foreach(transactions, headers: :true, header_converters: :symbol) do |row|
    Transaction.create!(id: row[:id], result: row[:result], invoice_id: row[:invoice_id], credit_card_number: row[:credit_card_number], created_at: row[:created_at], updated_at: row[:updated_at])
  end
  ...
  CSV.foreach(items, headers: :true, header_converters: :symbol) do |row|
    Item.create!(id: row[:id], name: row[:name], description: row[:description], unit_price: BigDecimal.new(row[:unit_price].insert(-3, ".")), merchant_id: row[:merchant_id], created_at: row[:created_at], updated_at: row[:updated_at])
  end
